...
export const registerUserHandler = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    console.log(
      `[Auth Controller] Registering user '${email}' successful: ${userId}`,
    );
    ...
    console.log(
      `[Auth Controller] Registering user '${username}' to user db successful`,
    );
...
export const loginUserHandler = async (
  ...
  try {
    ...
    if (result.twoFA) {
      console.log(
        `[Auth Controller] User ${result.userId} ${email} has 2FA enabled`,
      );
      const twoFAPayload = {
        success: true,
        ...
        twoFA: true,
        message:
          "Two-factor authentication is enabled for this user. Please verify your token.",
      };
      reply.status(200).send(AuthResultSchema.parse(twoFAPayload));
    }

    console.log(
      `[Auth Controller] Handling successful login for user ${result.userId} ${email}`,
    );
    await handleSuccessfulLogin(request, reply, Number(result.userId));
    console.log(
      `[Auth Controller] User ${result.userId} ${email} logged in successfully`,
    );
...
export const OAuthloginHandler = async (
  ...
  try {
    ...
    if (!redirectUri || !cliendID) {
      console.error(
        `[Auth Controller] OAuth redirect URI or client ID is not configured`,
      );
      reply.status(500).send({
        success: false,
...
export const OAuthCallbackHandler = async (
  ...
  try {
    ...
    console.log(
      `[Auth Controller] User ${userId} logged in successfully after OAuth callback`,
    );
...
export const verify2FATokenHandler = async (
  request: FastifyRequest,
  reply: FastifyReply,
) => {
  try {
    ...
    console.log(
      `[Auth Controller] Received request to verify 2FA token for user ${email}`,
    );
    if (!token || !email) {
    ...
    if (userId === undefined) {
      console.error(
        `[Auth Controller] User ID missing after 2FA verification for user ${email}`,
      );
      reply
        .status(500)
        .send({ error: "User ID missing after 2FA verification" });
      return;
    }

    console.log(
      `[Auth Controller] User ${email} verified 2FA token successfully`,
    );

    console.log(
      `[Auth Controller] Handling successful login for user ${email}`,
    );
    await handleSuccessfulLogin(request, reply, userId);
    console.log(
      `[Auth Controller] User ${email} logged in successfully after 2FA verification`,
    );
...
export const logoutUserHandler = async (
  ...
  try {
    ...
    console.log(
      `[Auth Controller] Cleared cookie for user ${request.user.userId}`,
    );
...
export const updatePasswordHandler = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    console.log(
      `[Auth Controller] Updating password for user ${userId} successful`,
    );
...
export const enable2FAHandler = async (
  ...
  try {
    ...
    console.log(
      `[Auth Controller] Enabling 2FA for user ${request.user.userId}`,
    );
    const result = await enable2FA(request.user.userId);

    if (!result.success) {
      console.error(
        `[Auth Controller] Failed to enable 2FA for user ${request.user.userId}: ${result.error}`,
      );
    ...
    console.log(
      `[Auth Controller] 2FA enabled successfully for user ${request.user.userId}`,
    );
