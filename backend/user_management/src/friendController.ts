...
const handleFriendRequestError = (reply: FastifyReply, error: string) => {
  const errorResponses: Record<string, { status: number; message: string }> = {
    ...
    REQUEST_ALREADY_RECEIVED: {
      status: 409,
      message: "User requested you already, check Requests",
    },
    CANNOT_FRIEND_SELF: {
      status: 400,
...
export const friendRequestHandler = async (
  ...
  try {
    ...
    console.log(
      `[User Controller] Creating friend request from user ${request.user.userId} to ${friendId}`,
    );
    await createFriendRequest(request.user.userId, friendId);
    console.log(
      `[User Controller] Friend request created from user ${request.user.userId} to ${friendId}`,
    );
...
export const acceptFriendRequestHandler = async (
  ...
  try {
    ...
    console.log(
      `[User Controller] Accepting friend request for ${request.user.userId} of ${userIdSender}`,
    );
    await acceptFriendRequest(request.user.userId, userIdSender);
    console.log(
      `[User Controller] Accepting friend request for ${request.user.userId} of ${userIdSender} successful`,
    );
...
export const rejectFriendRequestHandler = async (
  ...
  try {
    ...
    console.log(
      `[User Controller] Removing friend request for ${request.user.userId} of ${userIdSender}`,
    );
    await removeFriend(request.user.userId, userIdSender);
    console.log(
      `[User Controller] Removing friend request for ${request.user.userId} of ${userIdSender} successful`,
    );
...
export const getFriendRequestsHandler = async (
  ...
  try {
    console.log(
      `[User Controller] Getting pending friend requests for user ${request.user.userId}`,
    );
    const friendRequests: Friendship[] = await getFriendRequests(
      request.user.userId,
    );
    console.log(
      `[User Controller] Getting pending friend requests for user ${request.user.userId} successful`,
    );
    ...
    reply
      .status(200)
      .send(FriendRequestListResponseSchema.parse(successPayload));
  } catch (e) {
    console.error("Error:", e);
...
export const getFriendsHandler = async (
  ...
  try {
    console.log(
      `[User Controller] Getting friend list for user ${request.user.userId}`,
    );
    const friends: Friend[] = await getFriendList(request.user.userId);
    console.log(
      `[User Controller] Getting friend list for user ${request.user.userId} successful`,
    );
...
export const removeFriendHandler = async (
  ...
  try {
    ...
    console.log(
      `[User Controller] Removing friend ${userIdToRemove} of ${request.user.userId}`,
    );
    await removeFriend(request.user.userId, userIdToRemove);
    console.log(
      `[User Controller] Removing friend ${userIdToRemove} for ${request.user.userId} successful`,
    );
