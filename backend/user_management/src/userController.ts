...
export const insertUserHandler = async (
  ...
  try {
    ...
    console.log(
      `[User controller] Inserting user with username '${username}' into db`,
    );
    await insertUser(username, userId);
    console.log(
      `[User controller] Inserting user '${username}' into db successful`,
    );
    ...
    if (e && e.code === "SQLITE_CONSTRAINT_UNIQUE") {
      ...
      reply.status(500).send({
        success: false,
        error:
          "An error occured inserting a new user into user_service database.",
      });
    }
  }
};
...
export const getUserDataHandler = async (
  ...
  try {
    console.log(
      "[User Controller] Getting user data from user db for:",
      request.user.userId,
    );
...
export const updateUsernameHandler = async (
  ...
  try {
    ...
    console.log(
      `[User Controller] Updating username in db for user ${request.user.userId} to '${newValue}'`,
    );
    await updateUsername(request.user.userId, newValue);
    console.log(
      `[User Controller] Updating username for user ${request.user.userId} successful`,
    );
...
export const updatePasswordHandler = async (
  ...
  try {
    ...
    console.log(
      `[User Controller] Updating password in auth db for user ${request.user.userId}`,
    );
    await updatePassword(request.user.userId, newValue);
    console.log(
      `[User Controller] Updating password for user ${request.user.userId} successful`,
    );
...
export const getUserIdByUsernameHandler = async (
  ...
  try {
    ...
    console.log(
      `[User Controller] Getting corresponding user id for user '${username}'`,
    );
    const userId = await getUserId(username);
    console.log(
      `[User Controller] Getting corresponding user id for user '${username}' successful: ${userId}`,
    );
...
export const getUsernameByUserIdHandler = async (
  request: FastifyRequest,
  reply: FastifyReply,
) => {
  try {
    ...
    console.log(
      `[User Controller] Getting corresponding username for user id '${userId}'`,
    );
    const username = await getUsername(userId);
    console.log(
      `[User Controller] Getting corresponding username for user id '${userId}' successful: ${username}`,
    );
...
export const getUserAvatarHandler = async (
  request: FastifyRequest,
  reply: FastifyReply,
) => {
  try {
    console.log(
      "[User Controller] Getting user avatar from user db for:",
      request.user.userId,
    );
    const avatarPath = await getUserAvatarPath(request.user.userId);
    console.log(
      "[User Controller] Getting user avatar successful:",
      avatarPath,
    );
...
export const updateUserAvatarHandler = async (
  request: FastifyRequest,
  reply: FastifyReply,
) => {
  try {
    ...
    const filePath = path.join(
      process.env["AVATAR_DIR_PATH"] as string,
      "user_uploads/",
      filename,
    );
    ...
    console.log(
      `[User Controller] Updating avatar in db for user ${request.user.userId}`,
    );
    await updateAvatar(request.user.userId, filePath, buffer);
    console.log(
      `[User Controller] Updating avatar for user ${request.user.userId} successful`,
    );
