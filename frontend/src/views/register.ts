...
export async function renderRegisterView(view: z.infer<typeof RegisterViewSchema>, app: App): Promise<void> {
  app.appContainer.innerHTML = await fetch("/views/register.html").then((res) => res.text());
  bindNavbar(app);
  bindForm({ formId: 'registrationForm',	url: '/auth/register', serviceName: 'Registration' });
  (void view);
}
